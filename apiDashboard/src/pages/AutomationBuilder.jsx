...
import { 
  ...
  Settings, Trash2, ChevronLeft, Layout,
  MousePointer2, Plus, ArrowRight, Activity,
  Clock, AlertTriangle, Code, Bot
} from 'lucide-react';
...
function BuilderCanvas({ onClose, automation }) {
  ...
  return (
    <div className="flex h-screen bg-[#f1f5f9] overflow-hidden font-sans">
      ...
      <div className="flex-1 relative h-full w-full" ref={reactFlowWrapper}>
        ...
        >
          ...
          <Panel position="top-right" className="flex items-center gap-3">
             <div className="flex bg-white/80 backdrop-blur-md p-1.5 rounded-2xl border border-slate-100 shadow-xl">
                <button 
      ...
      {/* Settings Sidebar */}
      <aside 
        className={`w-96 bg-white border-l border-slate-200 flex flex-col z-20 shadow-[-20px_0_40px_rgba(0,0,0,0.05)] transition-all duration-300 ${selectedNode || !selectedNode ? 'translate-x-0' : 'translate-x-full'}`}
      >
        <div className="p-8 border-b border-slate-100 flex items-center justify-between">
            <div className="flex items-center gap-4">
                <div className="w-10 h-10 bg-slate-50 rounded-2xl flex items-center justify-center text-slate-400">
                    {selectedNode ? <Settings size={20} /> : <Layout size={20} />}
                </div>
                <h3 className="text-lg font-black text-slate-800 tracking-tight">
                    {selectedNode ? 'Configuration' : 'Workflow Settings'}
                </h3>
            </div>
            {selectedNode && (
                <button onClick={() => setSelectedNode(null)} className="p-2 hover:bg-slate-50 rounded-xl text-slate-400">
                    <X size={20} />
                </button>
            )}
        </div>
                    ...
                    <div className="space-y-4">
                        ...
                        <div className="flex items-center gap-3">
                            <button 
                                ...
                                className={`flex-1 py-3 px-4 rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all ${automation?.status === 'active' ? 'bg-emerald-500 text-white shadow-lg shadow-emerald-500/20' : 'bg-slate-50 text-slate-400 border border-slate-100'}`}
                            >
                                Active
                            </button>
                            <button 
                                onClick={() => handleSave('paused')}
                                className={`flex-1 py-3 px-4 rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all ${automation?.status === 'paused' ? 'bg-amber-500 text-white shadow-lg shadow-amber-500/20' : 'bg-slate-50 text-slate-400 border border-slate-100'}`}
                            >
                                Paused
                            </button>
                        </div>
                        <p className="text-[9px] text-slate-400 font-medium px-1 italic">Active workflows will automatically reply to incoming messages.</p>
                    </div>
        ...
        {selectedNode && (
            <div className="p-8 border-t border-slate-100 flex items-center gap-4">
                ...
                }} className="flex-1 flex items-center justify-center gap-2 px-6 py-4 bg-red-50 text-red-600 text-[10px] font-black rounded-2xl hover:bg-red-100 transition-all uppercase tracking-widest">
                    <Trash2 size={14} />
                    Delete Node
                </button>
            </div>
