        ...
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
          ...
          <div className="flex items-center gap-3">
            ...
            <button 
              ...
              className={`flex items-center gap-2 px-8 py-3 rounded-xl text-sm font-bold transition-all shadow-lg active:scale-95 ${
                       ...
                       {['groups', 'contacts', 'excel'].map(mode => (
                         <button
                           ...
                           className={`px-4 py-1.5 text-[10px] font-black uppercase rounded-lg transition-all ${
                             formData.audienceMode === mode ? 'bg-white text-primary shadow-sm' : 'text-slate-400'
                           }`}
                         >
                           {mode}
                         </button>
                        ...
                        {groups.map(g => (
                          <button
                            ...
                            className={`p-4 rounded-2xl border-2 text-left transition-all flex items-center justify-between group ${
                              formData.group_ids.includes(g._id) ? 'border-secondary bg-secondary/5' : 'border-slate-50 bg-slate-50/50 hover:bg-white'
                            }`}
                          >
                             ...
                             {allContacts.filter(c => (c.name || '').toLowerCase().includes(contactSearch.toLowerCase()) || (c.phoneNumber || '').includes(contactSearch)).map(c => (
                               <button
                                 ...
                                 className={`p-3 rounded-xl border flex items-center gap-3 transition-all ${
                                   formData.contacts.includes(c.phoneNumber) ? 'border-primary bg-primary/5' : 'border-slate-50 bg-slate-50/50 hover:bg-white'
                                 }`}
                               >
                    ...
                    {templates.map(t => (
                      <button
                        ...
                        className={`p-5 rounded-2xl border-2 text-left transition-all relative ${
                          formData.template_id === t._id ? 'border-primary bg-primary/5' : 'border-slate-50 bg-slate-50/50 hover:bg-white'
                        }`}
                      >
