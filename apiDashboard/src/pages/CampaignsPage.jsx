...
import {
  ...
  BarChart2, Target, Users, LayoutDashboard, X, RefreshCcw, Upload
} from 'lucide-react';
      ...
      <main className="flex-1 overflow-y-auto p-4 md:p-6 space-y-6 custom-scrollbar pb-10">
        ...
        <div className="flex items-center justify-between mb-6">
          ...
          <button
            ...
            className="flex items-center justify-center gap-2 px-6 py-3 bg-secondary text-white text-sm font-bold rounded-2xl hover:brightness-110 transition-all shadow-xl shadow-secondary/25 active:scale-95 self-start md:self-auto"
          >
            <Plus size={16} />
            New Campaign
          </button>
          ...
          <div className="flex flex-col md:flex-row gap-4 items-center justify-between">
            <div className="relative w-full md:w-96">
              ...
              <input
                ...
                className="w-full pl-11 pr-4 py-2.5 bg-slate-50 border border-slate-100 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-primary/10 focus:border-primary transition-all shadow-sm"
              />
            </div>
            ...
            <div className="flex items-center gap-1 bg-slate-100 p-1 rounded-xl ml-auto md:ml-0">
              ...
              <button
                ...
                className={`p-2 rounded-lg transition-all ${viewMode === 'table' ? 'bg-white text-primary shadow-sm' : 'text-slate-400 hover:text-slate-600'}`}
                title="Table View"
              >
                <Menu size={18} />
              </button>
...
function CampaignRow({ campaign, onStatusUpdate }) {
  ...
  return (
    <div className="bg-white rounded-2xl border border-slate-100 hover:border-primary/20 hover:bg-slate-50/30 transition-all p-5 flex flex-col lg:flex-row lg:items-center gap-6 group relative overflow-hidden">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-6 md:gap-10 px-0 md:px-6 border-l-0 md:border-l border-slate-100">
        ...
        <StatItem label="Sent" value={campaign.sent_count || 0} color="primary" />
        <StatItem label="Deliv" value={campaign.delivered_count || 0} color="secondary" />
      ...
      <div className="flex items-center gap-3 border-l border-slate-100 pl-6">
        {campaign.status === 'pending' || campaign.status === 'scheduled' ? (
          <button
            ...
            className="w-9 h-9 rounded-xl bg-secondary/10 text-secondary flex items-center justify-center hover:bg-secondary hover:text-white transition-all shadow-sm group/btn"
          ...
          <button
            disabled={isUpdating}
            onClick={() => handleAction('running')}
            className="w-9 h-9 rounded-xl bg-primary/10 text-primary flex items-center justify-center hover:bg-primary hover:text-white transition-all shadow-sm group/btn"
        ...
        <button className="w-9 h-9 rounded-xl bg-slate-50 text-slate-400 flex items-center justify-center hover:bg-slate-100 hover:text-primary transition-all border border-slate-100">
...
function CreateCampaignModal({ onClose, onSuccess }) {
  ...
  const [audienceMode, setAudienceMode] = useState('groups'); // 'groups', 'contacts', or 'excel'
  ...
  const totalRecipients = audienceMode === 'excel'
    ? (excelData?.rows.length || 0)
    : (audienceMode === 'groups'
      ? groups.filter(g => formData.group_ids.includes(g._id)).reduce((acc, curr) => acc + (curr.contacts?.length || 0), 0)
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 md:p-6 bg-slate-900/60 backdrop-blur-md animate-in fade-in duration-300">
      <div className="bg-white w-full max-w-5xl rounded-[2.5rem] shadow-2xl overflow-hidden animate-in zoom-in-95 duration-300 border border-white/20 max-h-[92vh] flex flex-col">
        {/* Modal Header */}
        <div className="px-8 py-6 border-b border-slate-100 flex items-center justify-between bg-white sticky top-0 z-10 shrink-0">
          <div className="flex items-center gap-5">
            ...
            <div>
              <h2 className="text-xl font-black text-slate-900 tracking-tight">Launch Campaign</h2>
              <p className="text-[11px] text-slate-400 font-bold uppercase tracking-widest mt-1">Design and broadcast your message</p>
            </div>
          </div>
          <button
            onClick={onClose}
            className="w-10 h-10 rounded-xl hover:bg-slate-50 flex items-center justify-center text-slate-400 transition-all hover:rotate-90"
          >
            <X size={20} />
          </button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar bg-slate-50/30">
          <div className="grid grid-cols-1 lg:grid-cols-12 h-full">
            ...
            <div className="lg:col-span-8 p-8 md:p-10 space-y-10 bg-white">
              ...
              <section className="space-y-6">
                <div className="flex items-center gap-3">
                  ...
                  <h3 className="text-sm font-black text-slate-900 uppercase tracking-widest">Campaign Basics</h3>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="space-y-2">
                    ...
                    <input
                      ...
                      className="w-full px-5 py-3.5 bg-slate-50 border border-transparent rounded-2xl text-sm font-bold focus:bg-white focus:ring-4 focus:ring-primary/5 focus:border-primary transition-all placeholder:text-slate-300"
                    />
                  </div>
                  <div className="space-y-2">
                    ...
                    <div className="relative group">
                      ...
                      <input
                        ...
                        className="w-full pl-12 pr-5 py-3.5 bg-slate-50 border border-transparent rounded-2xl text-sm font-bold focus:bg-white focus:ring-4 focus:ring-primary/5 focus:border-primary transition-all"
                      />
                    </div>
                  </div>
                </div>
              ...
              <section className="space-y-6">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    <span className="w-8 h-8 rounded-xl bg-secondary/10 text-secondary flex items-center justify-center text-xs font-black">02</span>
                  ...
                  <div className="flex bg-slate-100/80 p-1 rounded-xl">
                    {['groups', 'contacts', 'excel'].map((mode) => (
                      <button
                        ...
                        className={`px-4 py-1.5 text-[10px] font-black rounded-lg transition-all uppercase tracking-widest ${audienceMode === mode ? 'bg-white text-primary shadow-sm' : 'text-slate-400 hover:text-slate-600'}`}
                      >
                        {mode}
                      </button>
                    ))}
                  ...
                  {audienceMode === 'groups' ? (
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 max-h-[280px] overflow-y-auto pr-2 custom-scrollbar">
                      {groups.map(g => (
                        <button
                          ...
                          className={`p-4 rounded-2xl border-2 text-left transition-all flex items-center justify-between group ${formData.group_ids.includes(g._id) ? 'border-secondary bg-secondary/5' : 'border-slate-50 bg-slate-50/50 hover:border-slate-100 hover:bg-white'}`}
                        >
                          <div className="flex items-center gap-4">
                            <div className={`w-10 h-10 rounded-xl flex items-center justify-center transition-colors ${formData.group_ids.includes(g._id) ? 'bg-secondary text-white' : 'bg-white text-slate-300'}`}>
                              <Users size={18} />
                            </div>
                            <div>
                              <p className={`text-sm font-black transition-colors ${formData.group_ids.includes(g._id) ? 'text-primary' : 'text-slate-700'}`}>{g.name}</p>
                              <p className="text-[10px] text-slate-400 font-bold uppercase tracking-tight">{g.contacts?.length || 0} Contacts</p>
                            </div>
                          </div>
                          {formData.group_ids.includes(g._id) && (
                            <div className="w-5 h-5 bg-secondary text-white rounded-full flex items-center justify-center animate-in zoom-in duration-200">
                    ...
                    <div className="space-y-4">
                      <div className="relative">
                        ...
                        <input
                          ...
                          className="w-full pl-11 pr-4 py-3 bg-slate-50 border border-transparent rounded-2xl text-sm font-bold focus:bg-white focus:ring-4 focus:ring-primary/5 focus:border-primary outline-none"
                        />
                      </div>
                    ...
                    <div className="space-y-4">
                      <div
                        ...
                        className={`p-10 border-2 border-dashed rounded-[2.5rem] flex flex-col items-center justify-center gap-4 transition-all cursor-pointer ${excelData ? 'border-secondary/20 bg-secondary/5' : 'border-slate-200 bg-slate-50/50 hover:border-primary/20 hover:bg-primary/5'}`}
                      >
                        <input type="file" id="excel-upload-modal" accept=".xlsx, .xls, .csv" onChange={handleFileUpload} className="hidden" />
                        {excelData ? <CheckCircle2 size={32} className="text-secondary" /> : <Upload size={32} className="text-slate-300" />}
                        <div className="text-center">
                          <p className="text-xs font-black text-slate-700 uppercase tracking-widest">{excelData ? `${excelData.rows.length} Contacts Ready` : 'Upload Excel Sheet'}</p>
                          <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest mt-1">Drop file or click to browse</p>
                        </div>
                      </div>
                    </div>
                  )}
              ...
              <section className="space-y-6">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    <span className="w-8 h-8 rounded-xl bg-blue-100 text-blue-600 flex items-center justify-center text-xs font-black">03</span>
                    <h3 className="text-sm font-black text-slate-900 uppercase tracking-widest">Choose Template</h3>
                  </div>
                  <button onClick={handleSyncTemplates} disabled={loading} className="text-[10px] font-black text-primary uppercase tracking-widest flex items-center gap-2 hover:underline">
                    <RefreshCcw size={12} className={loading ? 'animate-spin' : ''} />
                    Sync with Meta
                  </button>
                </div>

                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 max-h-[320px] overflow-y-auto pr-2 custom-scrollbar p-1">
                  {templates.map(t => (
                    <button
                      key={t._id}
                      onClick={() => {
                      ...
                      className={`p-5 rounded-3xl border-2 text-left transition-all relative overflow-hidden group ${formData.template_id === t._id ? 'border-primary bg-primary/5' : 'border-slate-50 bg-slate-50/50 hover:border-slate-100 hover:bg-white'}`}
                    >
                      <div className="flex items-center justify-between mb-3">
                        <div className={`w-10 h-10 rounded-xl flex items-center justify-center transition-colors ${formData.template_id === t._id ? 'bg-primary text-white' : 'bg-white text-slate-400 border border-slate-100'}`}>
                          <MessageSquare size={18} />
                        </div>
                        <span className={`px-2 py-0.5 rounded-lg text-[8px] font-black uppercase tracking-widest border ${t.status === 'APPROVED' ? 'bg-secondary/10 text-secondary border-secondary/20' : 'bg-slate-100 text-slate-400 border-slate-200'}`}>
                          {t.status}
                        </span>
                      </div>
                      <h4 className={`text-sm font-black truncate transition-colors ${formData.template_id === t._id ? 'text-primary' : 'text-slate-800'}`}>{t.name}</h4>
            ...
            {/* Right Column - Preview & Personalization */}
            <div className="lg:col-span-4 bg-slate-50/50 p-8 flex flex-col border-l border-slate-100 overflow-y-auto max-h-[750px] lg:max-h-full">
              <div className="space-y-10">
                ...
                {selectedTemplate && (
                  <div className="space-y-6">
                    <div className="flex items-center gap-3">
                      <span className="w-8 h-8 rounded-xl bg-orange-100 text-orange-600 flex items-center justify-center text-xs font-black">04</span>
                      <h3 className="text-sm font-black text-slate-900 uppercase tracking-widest">Personalize</h3>
                    </div>
                    ...
                    <div className="space-y-5">
                      {(() => {
                        ...
                        if (uniqueVars.length === 0) return <p className="text-[11px] text-slate-400 font-bold italic text-center py-4 bg-white rounded-2xl border border-slate-100">No variables to configure</p>;

                        return uniqueVars.map((v, idx) => (
                          <div key={idx} className="space-y-2">
                            <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest ml-1">Variable {v}</label>
                            {audienceMode === 'excel' ? (
                              <select
                                className="w-full px-4 py-3 bg-white border border-slate-200 rounded-xl text-xs font-bold focus:ring-4 focus:ring-primary/5 focus:border-primary outline-none"
                              ...
                              <div className="flex flex-col gap-2">
                                <select
                                  className="w-full px-4 py-2.5 bg-white border border-slate-100 rounded-xl text-xs font-bold focus:ring-4 focus:ring-primary/5 focus:border-primary outline-none"
                                  onChange={(e) => {
                                ...
                                {!formData.variable_values[idx]?.startsWith('{{contact.') && (
                                  <input
                                    ...
                                    className="w-full px-4 py-2.5 bg-white border border-slate-100 rounded-xl text-xs font-bold focus:ring-4 focus:ring-primary/5 focus:border-primary transition-all placeholder:text-slate-300"
                                  />
                                )}
                              </div>
                            )}
                ...
                {/* Live Preview */}
                <div className="space-y-6">
                  <h4 className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] text-center">Live Preview</h4>
                  <div className="relative w-full max-w-[260px] mx-auto aspect-[1/2] bg-slate-900 rounded-[2.5rem] border-[6px] border-slate-800 shadow-2xl overflow-hidden p-2">
                    <div className="absolute inset-0 bg-[#e5ddd5] opacity-20 pointer-events-none"></div>
                    <div className="h-full bg-white/40 backdrop-blur-sm rounded-3xl p-3 flex flex-col justify-end">
                      <div className="bg-white rounded-xl shadow-sm p-3 space-y-2 animate-in slide-in-from-bottom-2">
                        <div className="text-[10px] text-slate-800 leading-relaxed font-medium">
                          {selectedTemplate ? (selectedTemplate.content?.body || selectedTemplate.content || "").replace(/\{\{([^}]+)\}\}/g, "...") : "Select a template..."}
                        </div>
                        <div className="flex justify-end">
                          <span className="text-[8px] text-slate-300 font-bold">12:45 PM</span>
                        </div>
                      </div>
                    </div>
                  </div>
        ...
        {/* Modal Footer */}
        <div className="px-8 py-6 bg-white border-t border-slate-100 flex flex-col sm:flex-row items-center justify-between gap-6 shrink-0">
          <div className="flex flex-col text-center sm:text-left">
            <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.15em]">Estimated Audience</p>
            <div className="flex items-center justify-center sm:justify-start gap-2 mt-0.5">
              <span className="text-xl font-black text-primary">{totalRecipients.toLocaleString()}</span>
              <span className="text-xs font-bold text-slate-500 uppercase tracking-tight">Recipients</span>
            </div>
          </div>
          ...
          <div className="flex items-center gap-4 w-full sm:w-auto">
            <button
              onClick={onClose}
              className="flex-1 sm:flex-none px-8 py-3.5 text-[11px] font-black text-slate-400 hover:text-slate-600 uppercase tracking-widest transition-colors"
            >
              Discard
            </button>
            <button
              disabled={loading || !formData.campaign_name || !formData.template_id || totalRecipients === 0}
              onClick={handleSubmit}
              className="flex-1 sm:flex-none px-12 py-4 bg-primary text-white text-[11px] font-black rounded-2xl hover:brightness-110 transition-all shadow-xl shadow-primary/25 active:scale-95 flex items-center justify-center gap-3 uppercase tracking-widest disabled:opacity-30 disabled:grayscale"
            >
              {loading ? <Loader2 size={18} className="animate-spin" /> : <Send size={18} fill="currentColor" />}
              {loading ? 'Processing...' : 'Launch Broadcast'}
            </button>
          </div>
        </div>
      </div>
