...
const StatCard = ({ title, value, change, trend, icon: Icon, color }) => (
  <div className="p-8 bg-white rounded-[2.5rem] border border-slate-100 shadow-xl shadow-slate-200/20 relative overflow-hidden group hover:scale-[1.02] transition-all duration-500">
    <div className={`absolute top-0 right-0 w-32 h-32 bg-${color}-500/5 rounded-full -mr-16 -mt-16 blur-2xl group-hover:bg-${color}-500/10 transition-all duration-700`}></div>
    <div className="relative z-10 space-y-4">
      <div className="flex items-center justify-between">
        <div className={`w-14 h-14 bg-${color}-50 rounded-2xl flex items-center justify-center text-${color}-600 shadow-sm border border-${color}-100`}>
          <Icon size={28} />
        </div>
        <div className={`flex items-center gap-1 px-3 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest ${trend === 'up' ? 'bg-emerald-50 text-emerald-600' : 'bg-red-50 text-red-600'}`}>
  ...
  return (
    <div className="h-full bg-[#F8FAFC] flex flex-col p-8 lg:p-12 space-y-10 overflow-y-auto custom-scrollbar">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div className="space-y-2">
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-4xl font-black text-slate-900 tracking-tight">Performance <span className="text-indigo-600">Analytics</span></h1>
          </div>
          ...
          <div className="flex bg-slate-50 p-1 rounded-xl border border-slate-100">
            {['7D', '30D', '90D', '1Y'].map(period => (
              <button 
                ...
                className={`px-4 py-2 rounded-lg text-[10px] font-black uppercase tracking-widest transition-all ${range === period ? 'bg-white text-indigo-600 shadow-sm' : 'text-slate-400 hover:text-slate-600'}`}
              >
                {period}
              </button>
            ))}
          </div>
          <button className="flex items-center gap-2 px-5 py-3 bg-indigo-600 text-white text-[10px] font-black uppercase tracking-widest rounded-xl hover:bg-indigo-700 transition-all shadow-lg shadow-indigo-600/10">
            <Download size={16} />
            Export Report
          </button>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
        <StatCard title="Total Sent" value={data?.kpi?.messagesSent || '0'} change="+14.2%" trend="up" icon={MessageSquare} color="indigo" />
        <StatCard title="Delivery Rate" value={data?.kpi?.deliveryRate || '0%'} change="+0.4%" trend="up" icon={CheckCircle} color="emerald" />
          ...
          <div className="h-[400px] w-full">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={performanceData}>
                ...
                <Area type="monotone" dataKey="delivered" stroke="#10b981" strokeWidth={4} fillOpacity={1} fill="url(#colorRead)" />
              </AreaChart>
            </ResponsiveContainer>
          </div>
        </div>
                ...
                <tr key={i} className="group hover:bg-slate-50/50 transition-all">
                  <td className="py-6">
                    <div className="flex items-center gap-4">
                      <div className="w-10 h-10 bg-indigo-50 rounded-xl flex items-center justify-center text-indigo-600 font-black text-[10px]">#{i+1}</div>
                  ...
                  <td className="py-6">
                    <div className="flex items-center gap-2">
                      <div className="w-16 h-1.5 bg-slate-100 rounded-full overflow-hidden">
                        <div className="h-full bg-emerald-500 rounded-full" style={{ width: campaign.read }}></div>
                      </div>
                      <span className="text-[10px] font-black text-emerald-600 tracking-tight">{campaign.read}</span>
                    </div>
                  </td>
                  <td className="py-6 text-xs font-black text-slate-600 tracking-tight">{campaign.conv}</td>
                  <td className="py-6 text-right">
                    <span className="px-4 py-2 bg-indigo-50 text-indigo-600 text-[11px] font-black rounded-lg border border-indigo-100">{campaign.roi}</span>
                  </td>
                </tr>
              ))}
            </tbody>
