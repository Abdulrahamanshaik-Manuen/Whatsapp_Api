...
export default function ForgotPasswordPage({ onNavigate }) {
  ...
  return (
    <div className="h-screen w-full flex bg-white font-['Inter',_sans-serif] overflow-hidden select-none">
      ...
      <div style={{ width: '45%', minWidth: '460px', background: 'linear-gradient(145deg, #002a52 0%, #003B6D 60%, #004f94 100%)', position: 'relative', overflow: 'hidden', display: 'flex', flexDirection: 'column', padding: '36px 40px', height: '100vh' }}>
        ...
        <div style={{ marginBottom: '24px', position: 'relative', zIndex: 1 }}>
          <h1 style={{ fontFamily: 'Manrope, sans-serif', fontSize: '1.9rem', fontWeight: 900, lineHeight: 1.2, color: '#fff', margin: '0 0 8px 0' }}>
            Account <span style={{ color: '#63C132' }}>Recovery</span><br />
            Secure & Simple
          </h1>
        ...
        <div style={{ position: 'relative', zIndex: 1, flex: 1, display: 'flex', flexDirection: 'column', gap: '16px' }}>
          <div style={{ background: 'rgba(255,255,255,0.07)', border: '1px solid rgba(255,255,255,0.12)', borderRadius: '16px', padding: '20px' }}>
             <div style={{ width: '48px', height: '48px', background: 'rgba(99,193,50,0.1)', borderRadius: '12px', display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: '16px' }}>
               <ShieldCheck size={24} color="#63C132" />
             </div>
      ...
      <div className="flex-1 flex flex-col p-8 lg:p-12 xl:p-24 relative justify-center bg-[#FAFAFB] h-full overflow-y-auto no-scrollbar">
        <div className="absolute top-10 left-12">
          <button onClick={() => onNavigate('/login')} className="flex items-center gap-2 text-sm text-slate-500 hover:text-[#003B6D] font-bold transition-colors">
        ...
        <div className="max-w-[400px] w-full mx-auto">
          ...
          {step === 1 && (
            <div className="space-y-6">
              <div>
                <h3 className="text-[32px] font-black text-[#003B6D] mb-2 font-['Plus_Jakarta_Sans',_sans-serif] tracking-tight">Forgot Password?</h3>
              ...
              <div className="space-y-2">
                ...
                <div className="relative">
                  ...
                  <input type="tel" placeholder="Enter mobile number" value={phone} onChange={e => setPhone(e.target.value)}
                    className="w-full pl-12 pr-4 py-4 border border-slate-200 rounded-xl bg-white focus:outline-none focus:border-[#63C132] focus:ring-[4px] focus:ring-[#63C132]/5 transition-all text-sm font-medium" />
                </div>
              </div>
              <button onClick={handleSendOTP} disabled={loading} className="w-full bg-[#63C132] text-white font-bold py-4 rounded-xl shadow-lg flex items-center justify-center gap-2 mt-4 hover:brightness-105 active:scale-[0.98] transition-all">
          ...
          {step === 2 && (
            <div className="space-y-6">
              <div>
                <h3 className="text-[32px] font-black text-[#003B6D] mb-2 font-['Plus_Jakarta_Sans',_sans-serif] tracking-tight">Verify Identity</h3>
                <p className="text-slate-500 font-medium text-base">Enter the 6-digit code sent to <span className="text-[#003B6D] font-bold">{phone}</span></p>
              </div>
              <div className="flex gap-2 justify-center">
                {otp.map((d, i) => (
                  <input key={i} ref={otpRefs[i]} value={d} maxLength={1} inputMode="numeric"
                    ...
                    className="w-12 h-14 text-center text-xl font-black border-2 border-slate-200 rounded-xl text-[#003B6D] bg-white focus:outline-none focus:border-[#63C132] transition-all" />
                ))}
              </div>
              <p className="text-center text-xs text-slate-400">
                Didn't receive the code? {' '}
                {timer > 0 ? <span className="text-[#63C132] font-bold">Resend OTP ({timer}s)</span> : <button onClick={handleSendOTP} className="text-[#63C132] font-bold hover:underline">Resend OTP</button>}
              </p>
              <button onClick={handleVerifyOTP} className="w-full bg-[#63C132] text-white font-bold py-4 rounded-xl shadow-lg flex items-center justify-center gap-2 mt-4 transition-all">
          ...
          {step === 3 && (
            <div className="space-y-6">
              <div>
                <h3 className="text-[32px] font-black text-[#003B6D] mb-2 font-['Plus_Jakarta_Sans',_sans-serif] tracking-tight">Set New Password</h3>
                ...
                <div className="space-y-2">
                  ...
                  <div className="relative">
                    ...
                    <input type={showPassword ? 'text' : 'password'} placeholder="At least 8 characters" value={newPassword} onChange={e => setNewPassword(e.target.value)}
                      className="w-full pl-12 pr-12 py-4 border border-slate-200 rounded-xl bg-white focus:outline-none focus:border-[#63C132] transition-all text-sm font-medium" />
                ...
                <div className="space-y-2">
                  ...
                  <div className="relative">
                    ...
                    <input type="password" placeholder="Repeat your new password" value={confirmPassword} onChange={e => setConfirmPassword(e.target.value)}
                      className="w-full pl-12 pr-4 py-4 border border-slate-200 rounded-xl bg-white focus:outline-none focus:border-[#63C132] transition-all text-sm font-medium" />
                  </div>
                </div>
              </div>
              <button onClick={handleResetPassword} disabled={loading} className="w-full bg-[#63C132] text-white font-bold py-4 rounded-xl shadow-lg flex items-center justify-center gap-2 mt-4 transition-all">
          ...
          {step === 4 && (
            <div className="text-center space-y-6">
              ...
              <div>
                <h3 className="text-2xl font-black text-[#003B6D] mb-2">Password Reset Successful!</h3>
              ...
              <button onClick={() => onNavigate('/login')} className="w-full bg-[#63C132] text-white font-bold py-4 rounded-xl shadow-lg transition-all">
                Login Now
              </button>
            </div>
          )}
