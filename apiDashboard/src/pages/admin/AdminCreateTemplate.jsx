...
import {
   ...
   Image as ImageIcon, FileText, Phone, Link2, X, Eye, EyeOff, User
} from 'lucide-react';
...
// Refined WhatsApp Mockup (Extracted from client CreateTemplatePage)
...
export default function AdminCreateTemplate({ onBack }) {
   ...
   const handleCreateSubmit = async () => {
      ...
      try {
         ...
         const payload = {
            ...
            mediaHandle: newTemplate.mediaHandle,
            previewUrl: newTemplate.previewUrl,
            clientId: newTemplate.clientId,
            directSubmit: true // Admin pushes directly to Meta
         };
   ...
   const getValidationErrors = () => {
      ...
      if (!newTemplate.clientId) {
         errors.push({ type: 'error', text: 'Target client account is required.' });
      }
      ...
      if (vars.length > 0) {
         ...
         if (!isSequential) {
            errors.push({ type: 'error', text: 'Variables are not sequential. Do not skip numbers.' });
         }
      }
   ...
   return (
      <div className="flex-1 h-full overflow-y-auto p-4 space-y-4 custom-scrollbar pb-12 bg-[#F8FAFC]">
         <div className="max-w-[1200px] mx-auto">
            
            {/* Header / Title Row */}
            <div className="flex flex-col sm:flex-row sm:items-center justify-between pb-3.5 border-b border-slate-100 gap-3 shrink-0 mb-4">
               <div className="flex items-center gap-3 min-w-0">
                  <button
                     onClick={onBack}
                     className="p-1.5 bg-white border border-slate-200 rounded-lg text-slate-400 hover:text-slate-600 transition-all active:scale-95 shadow-sm shrink-0"
                  >
                     <ChevronLeft size={16} />
                  </button>
                  <div className="min-w-0">
                     <h1 className="text-xl sm:text-2xl font-bold text-slate-800 tracking-tight leading-none">Create Template</h1>
                     <p className="text-xs text-slate-400 font-semibold mt-2 leading-none hidden sm:block">Design and push professional Meta templates for clients</p>
                  </div>
               </div>
               ...
               <div className="flex items-center gap-2 shrink-0">
                  <button
                     ...
                     className="flex items-center justify-center gap-1.5 h-8 px-3 sm:px-3.5 bg-white border border-slate-200 hover:bg-slate-50 text-slate-700 text-xs font-bold rounded-lg shadow-sm transition-all cursor-pointer"
                  >
                  ...
                  <button
                     ...
                     className="flex items-center gap-1.5 h-8 px-3 sm:px-4 py-2 bg-[#003B6D] hover:opacity-90 text-white text-xs font-bold rounded-lg shadow-sm transition-colors disabled:opacity-50 cursor-pointer"
                  >
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-4 items-start">
               ...
               <div className={`${showPreview ? 'lg:col-span-8' : 'lg:col-span-12'} space-y-4 transition-all duration-300`}>
                  
                  {/* Card 1: Client Assignment */}
                  <div className="bg-white rounded-lg border border-slate-200/60 p-5 shadow-sm space-y-4">
                     ...
                     <div className="space-y-1.5">
                        <label className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Target Client Account</label>
                        <select
                           value={newTemplate.clientId}
                           onChange={e => setNewTemplate({ ...newTemplate, clientId: e.target.value })}
                           className="w-full px-3 py-2 bg-white border border-slate-200 rounded-lg text-xs font-semibold focus:ring-1 focus:ring-slate-350 focus:border-slate-400 outline-none transition-all cursor-pointer shadow-sm"
                        >
                  ...
                  <div className="bg-white rounded-lg border border-slate-200/60 p-5 shadow-sm space-y-4">
                     <h3 className="text-sm font-bold text-slate-855 border-b border-slate-100 pb-2">Template Details</h3>
                     ...
                     <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                        ...
                        <div className="space-y-1.5">
                           ...
                           <input
                              ...
                              className="w-full px-3 py-2 bg-white border border-slate-200 rounded-lg text-xs font-semibold focus:outline-none focus:border-slate-400 outline-none"
                           />
                        </div>
                        ...
                        <div className="space-y-1.5">
                           ...
                           <select
                              ...
                              className="w-full px-3 py-2 bg-white border border-slate-200 rounded-lg text-xs font-semibold focus:outline-none focus:border-slate-400 outline-none transition-all cursor-pointer"
                           >
                        ...
                        <div className="pt-2 animate-in fade-in duration-200">
                           ...
                           >
                                 ...
                                 onChange={async (e) => {
                                    ...
                                    try {
                                       ...
                                       if (response.ok && data.previewUrl) {
                                          setNewTemplate(prev => ({ ...prev, mediaHandle: data.handle || '', previewUrl: data.previewUrl }));
                                       } else {
                              ...
                              {newTemplate.previewUrl ? (
                                 ...
                                 <>
                                    <div className="w-10 h-10 rounded-xl bg-white flex items-center justify-center text-slate-400 group-hover:text-slate-600 transition-colors shadow-sm"><UploadIcon size={16} /></div>
                     ...
                     <div className="space-y-3">
                        ...
                        <div className="flex items-center gap-2 bg-slate-50 p-1.5 rounded-lg border border-slate-200/50">
                           <button
                              ...
                              className="px-2.5 py-1 bg-white border border-slate-200 hover:bg-slate-100 rounded text-[10px] font-bold text-slate-700 transition-colors shadow-sm flex items-center gap-1 cursor-pointer"
                           >
                              <span>+</span>
                              <span>Variable</span>
                           </button>
                           <button
                              ...
                              className="px-2.5 py-1 bg-white border border-slate-200 hover:bg-slate-100 rounded text-[10px] font-bold text-slate-700 transition-colors shadow-sm cursor-pointer"
                           >
                              ☀️
                           </button>
                           <button
                        ...
                        <textarea
                           ...
                           rows={5}
                           className="w-full px-4 py-4 bg-white border border-slate-200 rounded-xl text-xs font-semibold leading-relaxed focus:border-slate-455 outline-none resize-none placeholder:text-slate-350"
                        />
                     </div>
                     ...
                     <div className="space-y-5">
                        ...
                        <div className="flex gap-2">
                           <button
                              ...
                              className="px-3 py-1.5 bg-slate-50 border border-slate-200 rounded-lg text-[10px] font-bold text-slate-600 hover:bg-slate-100 transition-colors cursor-pointer"
                           >
                              + Quick Reply
                           </button>
                           <button
                              ...
                              className="px-3 py-1.5 bg-slate-50 border border-slate-200 rounded-lg text-[10px] font-bold text-slate-655 hover:bg-slate-100 transition-colors cursor-pointer"
                           >
                              + Visit Website
                           </button>
                           <button
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3.5 mt-3">
                           {newTemplate.buttons.map((btn, idx) => (
                              <div key={idx} className="bg-slate-50/40 border border-slate-200 rounded-xl p-3 relative group">
                                 ...
                                 <span className="text-[9px] font-extrabold uppercase tracking-widest text-[#003B6D]">
                                 ...
                                 <div className="space-y-2 mt-2">
                                    <input
                                       ...
                                       className="w-full px-3 py-1.5 bg-white border border-slate-200 rounded-lg text-xs font-semibold focus:border-slate-450 outline-none"
                                    />
                                    ...
                                    {btn.type === 'URL' && (
                                       <input
                                          ...
                                          className="w-full px-3 py-1.5 bg-white border border-slate-200 rounded-lg text-xs font-semibold focus:border-slate-455 outline-none font-mono"
                                       />
                                    )}
